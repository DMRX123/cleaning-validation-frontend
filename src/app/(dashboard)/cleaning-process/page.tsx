// src/app/(dashboard)/cleaning-process/page.tsx
'use client'
...
import { Plus, Eye, TrendingUp, Loader2, Trash2 } from 'lucide-react'
...
export default function CleaningProcessPage() {
  ...
  const fetchProcesses = async () => {
    try {
      ...
      setProcesses(res.data.processes || res.data || [])
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Are you sure you want to delete this cleaning process?')) {
      try {
        await api.delete(`/cleaning-process/${id}`)
